...
def output(infn, docnum, sentences):
	...
	# Assume that your data is in brat directory called 'data/integration'
        txtout = codecs.open('data/integration/'+infn+'.txt', 'wt', encoding=OUTPUT_ENCODING)
        soout = codecs.open('data/integration/'+infn+'.ann', 'wt', encoding=OUTPUT_ENCODING)
    else:
	...
	attr_counter = 0
	    ...
	    spelling = correction(form)
		    ...
		    print >> soout,'A%d\tPossible_error T%d' % (attr_counter,idnum)
                    print >> soout,'#%d\tAnnotatorNotes T%d\t%s' % (idnum,idnum,POS)
		    attr_counter += 1
		if spelling:
		    print >> soout,'A%d\tSpelling_error T%d' % (attr_counter,idnum)
		    print >> soout,'#%d\tAnnotatorNotes T%d\t%s' % (idnum,idnum,spelling[0])
		    attr_counter += 1
...
def process(lines,docid):
    ...
    for ln,l in enumerate(lines):
        ...
        fields = l
        assert len(fields) == 6, "Format error on line %d in: %s" % (ln,l)
        (ID, token,form, POS, prob,real_prob) = (fields)
        #head, rel = fields[5], fields[5]

        tokens.append((ID, token, POS, prob,real_prob))
    ...
    if len(tokens) > 0:
        sentences.append((tokens, deps))
    if len(sentences) > 0:
        output(docid, docnum, sentences)

def standoff_main(words,docid):
    global output_directory
...
##############################################################################
#
# WARNING: error in processing %d/ files, output is incomplete!
#
##############################################################################
""" % (fail_count)
